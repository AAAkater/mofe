...
def train():
    ...
    dataset = RestorationDataset(
        "/root/autodl-tmp/data/train/CASIA-WebFace", max_samples=10000
    )
    ...
    criterion_perceptual = PerceptualLoss().to(
        config["device"]
    )  # 感知损失 使用 VGG 提取高层语义特征，保证结构一致（提升清晰度）
    ...
    # 训练循环
    for epoch in tqdm(
        range(config["epochs"]), desc="Training Epochs", total=config["epochs"]
    ):
        model.train()
        discriminator.train()
        total_loss_G = 0.0
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=3):
        ...
        self.model = nn.Sequential(
            *discriminator_block(
                in_channels * 2, 64, normalization=False
            ),  # 输入 6 通道
